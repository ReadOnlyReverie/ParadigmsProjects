...
somewhereLeftOf(A, B, List) :- 	% Reverse A and B for somewhere right of.
	nth1(IndexA, List, A),		% Call nth1 on elements A and B with the
    nth1(IndexB, List, B),		% condition that IndexA is less than IndexB.
    IndexA < IndexB.
    
between(A, B, C, List) :-		% Call nth1 on elements A, B, and C with
	nth1(IndexA, List, A),		% the condition that IndexA is less than
    nth1(IndexB, List, B),		% IndexB and IndexB is less than IndexC.
    nth1(IndexC, List, C),
    IndexA < IndexB,
    IndexB < IndexC.
    
% Use for clues that say directly next to without specifying left or right.
